...
def main(job_id, params):
    ...
    target.write("/maxIterations VALUE "+params["/maxIterations"][0]+"\n")
    target.write("/confidence VALUE "+params["/confidence"][0]+"\n")
    target.write("/k VALUE "+params["/k"][0]+"\n")
    target.write("/workerCount VALUE "+params["/workerCount"][0]+"\n")
    target.write("baseclass VALUE "+params["baseclass"][0])
    target.close()

    cmd = 'cd /home/user/pdeboer/PPLib && sbt "run-main ch.uzh.ifi.pdeboer.pplib.examples.optimization.MCOptimize /home/user/pdeboer/bo/Spearmint/examples/noisyPPLib_tal2/jobdesc_'+str(job_id)+'.txt"'
